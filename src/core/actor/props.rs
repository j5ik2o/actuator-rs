...
impl<Msg: Message> Props<Msg> for FunctionProps<Msg> {
  ...
  fn to_any(&self) -> Rc<dyn Props<AnyMessage>> {
    Rc::new(AnyProps::new(Rc::new(self.clone())))
    // let actor_f_clone = self.actor_f.clone();
    // let wrapper_rc = Rc::new(RefCell::new(AnyMessageActorFunctionWrapper::new(move || {
    //   (actor_f_clone.clone())()
    // })));
    // Rc::new(FunctionProps::new(move || wrapper_rc.clone()))
  }
}
