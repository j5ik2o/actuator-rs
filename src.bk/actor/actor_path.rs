...
impl ActorPath {
  ...
  /// ```rust
  /// # use oni_comb_uri_rs::models::uri::Uri;
  /// # use actuator_rs::actor::actor_path::ActorPath;
  ...
  /// ```rust
  /// # use actuator_rs::actor::actor_path::ActorPath;
  /// # use actuator_rs::actor::address::Address;
  /// let address = Address::new_with_host_port("actuator", "system", "localhost", 1234);
  /// let actor_path = ActorPath::of_root_with_name(address.clone(), "/user".to_string());
  ...
  /// ```rust
  /// # use actuator_rs::actor::address::Address;
  /// # use actuator_rs::actor::actor_path::{ActorPath, ActorPathBehavior};
  /// let address = Address::new_with_host_port("actuator", "system", "localhost", 1234);
  /// let actor_path = ActorPath::of_root_with_name(address.clone(), "/user".to_string());
  /// assert_eq!(actor_path.to_string(), "actuator://system@localhost:1234/user");
  /// ```
  ...
  /// # use actuator_rs::actor::address::Address;
  /// let address = Address::new_with_host_port("actuator", "system", "localhost", 1234);
  /// let root_path = ActorPath::of_root_with_name(address.clone(), "/root".to_string());
  /// let user_path = ActorPath::of_child(root_path.clone(), "user".to_string(), 0);
  /// let child_path = ActorPath::of_child(user_path.clone(), "child".to_string(), 0);
